...
public:
  ...
  [[nodiscard]] constexpr std::uint32_t VkCode() const noexcept {
    return value & 0xFF;
  }
  [[nodiscard]] constexpr bool HasCtrl() const noexcept {
    return (value & (1 << 8)) != 0;
  }
  [[nodiscard]] constexpr bool HasShift() const noexcept {
    return (value & (1 << 9)) != 0;
  }
  [[nodiscard]] constexpr bool HasAlt() const noexcept {
    return (value & (1 << 10)) != 0;
  }
  ...
  [[nodiscard]] bool IsPressed(UINT msg, WPARAM wp) const noexcept {
    ...
    return ctrl_pressed == HasCtrl() && shift_pressed == HasShift() &&
           alt_pressed == HasAlt();
  }
  ...
  [[nodiscard]] std::string ToString() const {
    if (IsEmpty()) {
      return std::string(
          tr("None", {{Language::Russian, "Нет"}, {Language::Chinese, "无"}})
              .view);
    }
    std::string result;
    if (HasCtrl())
...
bool FeedCapture(UINT msg, WPARAM wp) noexcept {
  ...
  if (vk == VK_CONTROL || vk == VK_SHIFT || vk == VK_MENU || vk == VK_LWIN ||
      vk == VK_RWIN || vk == VK_LCONTROL || vk == VK_RCONTROL ||
      vk == VK_LSHIFT || vk == VK_RSHIFT || vk == VK_LMENU || vk == VK_RMENU) {
    return true; // Ignore modifier only presses, but swallow them to prevent
                 // game processing
  }
...
bool RenderUi(const char *label, Hotkey &hotkey) {
  ...
  ImGui::SetItemTooltip(
      "%s",
      std::string(
          tr("Click to rebind. Press Esc to cancel, Delete or Backspace to "
             "clear.",
             {{Language::Russian, "Нажмите для переназначения. Esc для отмены, "
                                  "Delete или Backspace для очистки."},
              {Language::Chinese,
               "点击重新绑定。按 Esc 取消，按 Delete 或 Backspace 清除。"}})
              .view)
          .c_str());
  ...
  if (capture::just_captured && !is_waiting &&
      capture::target_hotkey == nullptr) {
    changed = true;
    capture::just_captured = false;
  }
